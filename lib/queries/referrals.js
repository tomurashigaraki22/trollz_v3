...
export async function getReferralStats(userId) {
  ...
  const [credits] = await query(
    "SELECT COUNT(*) AS successful_referral_count, COALESCE(SUM(credits_awarded), 0) AS credits_earned FROM referral_credit_log WHERE inviter_id = ?",
    [userId]
  );

  const referralCount = Number(referrals?.referral_count ?? 0);
  const successfulReferralCount = Number(credits?.successful_referral_count ?? 0);

  return {
    referralCount,
    pendingReferralCount: Math.max(referralCount - successfulReferralCount, 0),
    successfulReferralCount,
    creditsEarned: Number(credits?.credits_earned ?? 0),
  };
}
