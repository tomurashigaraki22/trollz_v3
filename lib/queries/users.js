...
export async function findUserByEmail(email) {
  // Case-insensitive: legacy rows and different signup/login attempts don't
  // always agree on casing (e.g. "John@Gmail.com" vs "john@gmail.com"), and
  // an exact-match lookup would silently report "incorrect password" for a
  // perfectly valid account.
  const rows = await query("SELECT * FROM users WHERE LOWER(email) = LOWER(?) LIMIT 1", [
    email.trim(),
  ]);
  return rows[0] ?? null;
}
...
export async function getAllCustomers() {
  return query(
    `SELECT u.id, u.name, u.email, u.phone, u.role, u.status, u.credits, u.loyalty_points,
