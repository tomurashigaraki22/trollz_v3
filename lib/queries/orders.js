...
import { formatOptionMap, serializeOptionMap } from "@/lib/productOptions";
            ...
            ${
              item.size || item.color || item.selected_options
                ? `<div style="margin-top:4px;color:#6b7280;font-size:13px;">
                    ...
                    ${item.selected_options ? `${item.size || item.color ? " | " : ""}${formatOptionMap(item.selected_options)}` : ""}
                  </div>`
                : ""
            }
          </td>
...
async function ensureOrderSchema() {
  ...
  if (!itemExisting.has("selected_options")) {
    await query("ALTER TABLE order_items ADD COLUMN selected_options TEXT NULL AFTER color");
  }
...
export async function createPendingOrder({
  ...
  return withTransaction(async (run) => {
    ...
    for (const item of items) {
      await run(
        `INSERT INTO order_items (order_id, product_id, product_name, price, quantity, size, color, selected_options, subtotal)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          orderId,
          item.productId,
          ...
          item.size ?? "",
          item.color ?? "",
          serializeOptionMap(item.options),
          item.price * item.qty,
        ]
      );
    }
