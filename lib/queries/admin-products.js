...
export async function addProduct({ item, category, subcategory, price, discount, qty, description, img, sizeOptions, colorOptions, attributes }) {
...
export async function updateProduct(id, { item, category, subcategory, price, discount, qty, description, img, sizeOptions, colorOptions, attributes }) {
  await ensureProductSchema();
  const oldPrice = price;
  const effectivePrice = discount > 0 ? Math.round(price * (1 - discount / 100)) : price;
  const sizeType = sizeOptions.length > 0 ? "custom" : "none";
  ...
  await query(
    `UPDATE product SET
      ...
      qty = ?, img = ?, old_price = ?, color_options = ?, size_type = ?, size_options = ?, attributes = ?
     WHERE id = ?`,
    [
      item,
      category,
