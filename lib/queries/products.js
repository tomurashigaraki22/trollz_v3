...
const STRUCTURED_ATTRIBUTE_KEYS = new Map([
  ["brand", "brand"],
  ["gender", "gender"],
  ["storage", "storage"],
  ["ram", "ram"],
...
function splitOptions(value, splitSlash = false) {
  const separator = splitSlash ? /[,|;/]+/ : /[,|;]+/;
  return [...new Set(String(value ?? "").split(separator).map((item) => item.trim()).filter(Boolean))];
}

function sellerProductIsFootwear(product) {
  return /shoe|footwear|sneaker|sandal|slipper|boot/i.test(
    `${product.name ?? ""} ${product.category ?? ""} ${product.subcategory ?? ""} ${product.description ?? ""}`
  );
}

export function parseSellerSpecifications(description, product = {}) {
  const isFootwear = sellerProductIsFootwear({ ...product, description });
  const sizeOptions = [];
  const colorOptions = [];
  const attributes = {};
  const cleanedLines = [];
  ...
  for (const line of String(description ?? "").split(/\r?\n/)) {
    ...
    const label = normalizeSpecificationLabel(match?.[1]);
    const value = match?.[2]?.trim() ?? "";
    const compactLabel = label.replace(/\s+/g, "");
    const isSizesLabel =
      label === "available sizes" ||
      label === "sizes" ||
      (isFootwear && ["size", "shoe size", "shoe sizes"].includes(label));
    const isColorsLabel = label === "available colors" || label === "colors" || label === "color";

    if (isSizesLabel && value) {
      sizeOptions.push(...splitOptions(value, true));
      continue;
    }
    if (isColorsLabel && value) {
      colorOptions.push(...splitOptions(value, true));
      continue;
    }
    if (match && value && STRUCTURED_ATTRIBUTE_KEYS.has(label)) {
      attributes[STRUCTURED_ATTRIBUTE_KEYS.get(label)] = value;
      continue;
    }
    if (compactLabel === "specifications" && !value) continue;
    cleanedLines.push(line);
  }

  return {
    description: cleanedLines.join("\n").replace(/\n{3,}/g, "\n\n").trim(),
    sizeOptions: [...new Set(sizeOptions)],
    colorOptions: [...new Set(colorOptions)],
    attributes,
    sizeType: sizeOptions.length > 0 ? (isFootwear ? "shoe" : "cloth") : "none",
  };
}
...
function parseProductRow(row) {
  ...
  const storedSizeOptions = row.size_type && row.size_type !== "none" ? safeJsonArray(row.size_options) : [];
  const isFootwear = /shoe|footwear|sneaker|sandal|slipper|boot/i.test(`${row.category ?? ""} ${row.subcategory ?? ""}`);
  const sizeOptions = storedSizeOptions.length > 0 ? storedSizeOptions : isFootwear ? SHOE_SIZES : [];
  const colorOptions = safeJsonArray(row.color_options);
  const attributes = safeJsonObject(row.attributes);
...
async function ensureSellerProductSyncSchema() {
  ...
  if (!(await columnExists("seller_products", "subcategory"))) {
    await query("ALTER TABLE seller_products ADD COLUMN subcategory VARCHAR(128) NULL");
  }
  if (!(await columnExists("product", "attributes"))) {
    await query("ALTER TABLE product ADD COLUMN attributes TEXT NULL");
  }
  sellerProductSyncReady = true;
}

function productImagesJson(value) {
...
async function resolveStorefrontCategory(categoryName, subcategoryName) {
  ...
  if (subcategory) {
    ...
    if (existing?.parent_id) {
      ...
      if (!category || parent.category.toLowerCase() === category.toLowerCase()) {
        return {
...
async function syncSellerProductToStorefront(product) {
  const category = await resolveStorefrontCategory(product.category, product.subcategory);
  const specifications = parseSellerSpecifications(product.description, product);
  const imageJson = productImagesJson(product.image_url);
    ...
    await query(
      `UPDATE product
       SET item = ?, category = ?, subcategory = ?, parent_category_id = ?, subcategory_id = ?,
           ...
           new = 1, img = ?, qty = ?, stock = ?, color_options = ?, size_type = ?, size_options = ?, attributes = ?
       WHERE id = ?`,
      [
        product.name,
        category.category,
        ...
        stock,
        stock,
        specifications.colorOptions.length > 0 ? JSON.stringify(specifications.colorOptions) : null,
        specifications.sizeType,
        specifications.sizeOptions.length > 0 ? JSON.stringify(specifications.sizeOptions) : null,
        JSON.stringify(specifications.attributes),
  ...
  const result = await query(
    `INSERT INTO product
      (item, category, subcategory, parent_category_id, subcategory_id, category_id,
       ...
       color_options, size_type, size_options, attributes, date)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 1, ?, ?, ?, ?, ?, ?, NOW())`,
    [
      product.name,
      category.category,
...
async function syncSellerProductByStorefrontId(id) {
  await ensureSellerProductSyncSchema();
  const products = await query(
    `SELECT sp.*, u.name AS seller_name, u.email AS seller_email
     FROM seller_products sp
     JOIN users u ON u.id = sp.seller_id
     WHERE sp.storefront_product_id = ?
     LIMIT 1`,
    [id]
  );
  if (products[0]) await syncSellerProductToStorefront(products[0]);
}
...
export async function getTrendingProducts(limit = 8) {
  // Keep the landing page fresh by choosing a new set of products on every
  // request instead of repeatedly showing the same highest-view products.
  const rows = await query(
    `SELECT p.*, r.avg_rating, r.review_count FROM product p
     ${REVIEW_JOIN}
     ORDER BY RAND()
...
export async function getProductById(id) {
  await syncSellerProductByStorefrontId(id);
  const rows = await query(
    `SELECT p.*, r.avg_rating, r.review_count FROM product p
     ${REVIEW_JOIN}
     WHERE p.id = ?
