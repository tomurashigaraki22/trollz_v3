  ...
  const products = await query(
    `SELECT sp.*, u.name AS seller_name, u.email AS seller_email
     ...
     WHERE sp.status <> 'draft'
       AND (sp.storefront_product_id IS NULL
         OR NOT EXISTS (SELECT 1 FROM product p WHERE p.id = sp.storefront_product_id))
     ORDER BY sp.id DESC
     LIMIT 50`
  );
    ...
    `SELECT p.*, r.avg_rating, r.review_count FROM product p
     ...
     ORDER BY
       CASE
         ...
         WHEN LOWER(p.item) = LOWER(?) THEN 2
         WHEN p.item LIKE ? THEN 3
         WHEN p.item LIKE ? THEN 4
         WHEN p.category LIKE ? OR p.subcategory LIKE ? THEN 5
         ELSE 6
       END,
       p.views DESC,
       r.avg_rating DESC
     LIMIT ?`,
    [like, like, like, like, cleanTerm, startsWith, cleanTerm, startsWith, like, like, like, limit]
  );
  return rows.map(parseProductRow);
}
