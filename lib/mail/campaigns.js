...
export async function sendTestEmailCampaign({ email, templateId, subject, body, ctaLabel, ctaUrl }) {
  const template = EMAIL_TEMPLATES.find((item) => item.id === templateId) ?? EMAIL_TEMPLATES[0];
  const recipientEmail = String(email || "").trim();
  const recipient = { email: recipientEmail, name: recipientEmail.split("@")[0] || "Test User" };
  const personalizedSubject = personalize(subject || template.subject, recipient);
